...
def extract_features(directory, sample_count):
  ...
  for inputs_batch, labels_batch in generator:
    ...
    if i * batch_size >= sample_count:
      # Breaks as the generator tries to yield data indefinitely
      break
...
# Flattens the 4 x 4 x 512 features into an array
train_features = np.reshape(train_features, (2000, 4 * 4 * 512))
validation_features = np.reshape(validation_features, (1000, 4 * 4 * 512))
test_features = np.reshape(test_features, (1000, 4 * 4 * 512))

# Adding the last layer and training it
...
# 90% accuracy but it overfits quickly as it does not use data augmentation